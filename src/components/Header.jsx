...
const Header = () => {
  ...
  return (
    <header className="relative w-full bg-gradient-to-r  to-yellow-600 hover:from-yellow-700 hover:to-indigo-700 active:from-purple-800 active:to-indigo-800  text-sm font-medium text-white font-sans">
      {/* Hero Banner */}
      <div className="relative w-full h-screen flex items-center justify-center ">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-6 max-w-4xl mx-auto">
          <h1 className="text-5xl md:text-7xl font-bold text-yellow-500 drop-shadow-xl">{data.hero_banner_heading}</h1>
          <h2 className="text-lg md:text-2xl text-yellow-600 drop-shadow-lg uppercase tracking-wide font-medium">{data.hero_banner_subheading}</h2>
          <p className="mt-2 text-lg md:text-xl text-yellow-200">{data.location} - {data.sublocation}</p>
          <p className="mt-1 text-lg md:text-xl text-yellow-100">{data.property_type_price_range_text} | {data.property_area_min_max}</p>
          <a href="#contact" className="mt-8 bg-gradient-to-r  to-yellow-600 hover:from-yellow-700 hover:to-indigo-700 active:from-purple-800 active:to-indigo-800 text-white px-8 py-3 rounded-full font-semibold text-lg shadow-xl transition-transform transform hover:scale-105">
            Enquire Now
          </a>
        </div>
      </div>
      ...
      <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'h-16 bg-black/90 backdrop-blur-md' : 'h-20 bg-gradient-to-b from-black/80 to-transparent'}`}>
        <div className="container mx-auto flex items-center justify-between h-full px-4">
          ...
          <div className="flex items-center space-x-4">
            <a href="#contact"
              className="hidden md:block bg-gradient-to-r from-yellow-500 to-yellow-600 
    hover:from-yellow-700 hover:to-indigo-700 
            ...
            <div className="px-6 py-4">
              ...
              <ul className="space-y-6">
                {[
                  ...
                  <li key={index}>
                    <a href={`#${item.href}`} onClick={() => setMenuOpen(false)} className="flex items-center text-gray-300 hover:text-yellow-400 transition-colors">
                      {item.icon}
                      {item.label}
                    </a>
                  </li>
              ...
              <div className="mt-10 pt-6 border-t border-gray-700">
                <a href="contact" onClick={() => setMenuOpen(false)} className="block text-center bg-yellow-500 text-white font-medium py-3 px-6 rounded-lg">Enquire Now</a>
              </div>
            </div>
          </div>
        </div>
